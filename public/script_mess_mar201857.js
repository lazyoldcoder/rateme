...
// const POPUP_HEIGHT = SCREEN_HEIGHT * 0.75;   // 75% of screen

// const OPEN = SCREEN_HEIGHT - POPUP_HEIGHT;   // visible position
// const CLOSED = -POPUP_HEIGHT;                // hidden position
...
// ======== Business Sheet Elements ========

// ======== Open / Close Sheet ========
function openSheet() {
  renderBusinessList(); // always refresh
  document.body.classList.add('sheet-open');
}
...
function renderBusinessList(list = businesses) {
  ...
  if (!list.length) {
    businessListEl.innerHTML = '<div class="business-row"><em>No businesses found</em></div>';
    return;
  }
  ...
  list.forEach(b => {
    ...
    row.addEventListener('click', () => selectBusiness(b.id));
    businessListEl.appendChild(row);
  });
}
...
function selectBusiness(id) {
  ...
  fetchRatings(); // load ratings for this business
  closeSheet();   // close popup
}
